...
- User wants to add a new task or goal to the project
- User asks to verify checklist items, run verification, or check what's done
- A skill completes and produces an artifact that should be tracked
- User asks for project status, progress report, or what's active
- Keywords: "add task", "track this", "verify checklist", "project status", "progress report", "paper audit", "审计论文", "写论文", "review loop", "完整流程"
...
+-- checklists/short-term.md          <- L1 (7 category sections each)
...
### Suggested Next
- Always -> `memory.write` (log creation)
