...
for script in recommend.py update_weights.py; do
  if [ -f "$SER_GLOBAL/scripts/$script" ]; then
    if [[ "$UPDATE_MODE" == true ]]; then
      cp "$SER_ROOT/../.claude/ser/scripts/$script" "$SER_GLOBAL/scripts/$script" 2>/dev/null || true
      echo "[~] Updated $script"
    else
...
for f in chains.yaml weights.yaml config.yaml; do
  if [ ! -f "$SER_GLOBAL/$f" ]; then
    cp "$SER_ROOT/../.claude/ser/$f" "$SER_GLOBAL/$f" 2>/dev/null || true
    echo "[+] Created $f"
  else
...
if [ ! -f "$SER_GLOBAL/td-nl/README.md" ]; then
  cp "$SER_ROOT/../.claude/ser/td-nl/README.md" "$SER_GLOBAL/td-nl/README.md" 2>/dev/null || true
