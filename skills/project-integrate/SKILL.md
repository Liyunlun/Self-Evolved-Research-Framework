...
description: Merge an unpacked SER distribution (ser-vX.Y/) into an existing research project — inventory conflicts, copy framework dirs, install the intent-router hook, merge .claude/settings.local.json and CLAUDE.md, create config.yaml, and seed memory. Triggers on "integrate SER", "set up SER for this project", "merge SER into this repo".
---
...
```bash
# From project root:
cp -r ser-vX.Y/skills .          # SER skill directories (including _shared/)
...
2. **Structure check**: Verify all SER skill directories exist under `skills/`
